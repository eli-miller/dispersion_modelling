...
sources = pd.DataFrame({'x': float(sources_df.DeltaY) + sources_x.ravel(),
...
retros_synthetic = pd.DataFrame({'x': X_synthetic.ravel(), 'y': Y_synthetic.ravel(), 'z': Z_synthetic.ravel()})
...
for stability in stablities:
    for i in tqdm.tqdm(range((len(Us)))):
        ...
        pics = get_synthetic_measurement(retros,
                                         sources,
                                         origin,
                                         u_mag=u,
                                         u_dir=u_dir,
